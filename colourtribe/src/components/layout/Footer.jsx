  ...
  return (
    <footer className="bg-navy text-white py-16 text-sm overflow-hidden" ref={ref}>
      ...
      >
        ...
        <motion.div variants={itemVariants}>
          ...
          <ul className="space-y-2 text-white/70">
            ...
            <li><Link to="/fabric-guide" className="hover:text-gold transition-colors flex items-center gap-2 group text-white/90 font-semibold"><span className="w-0 overflow-hidden group-hover:w-2 transition-all">›</span>Fabric Guide (AEO)</Link></li>
      ...
      <motion.div 
        ...
        className="container mx-auto px-8 mt-12 pt-8 border-t border-white/10"
      >
        <h4 className="text-white/40 uppercase tracking-widest text-xs font-bold mb-4 text-center">Serving Top Indian Cities</h4>
        <div className="flex flex-wrap justify-center gap-x-4 gap-y-2 text-white/50 text-xs">
          {['Delhi', 'Mumbai', 'Bangalore', 'Chennai', 'Kolkata', 'Hyderabad', 'Pune', 'Ahmedabad', 'Surat', 'Jaipur'].map(city => (
            <Link key={city} to={`/uniform-manufacturer-in-${city.toLowerCase()}`} className="hover:text-gold transition-colors">
              Uniforms in {city}
            </Link>
          ))}
        </div>
